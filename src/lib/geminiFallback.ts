// Direct Browser-to-Google-API Gemini Fallback Helper
// This ensures that even when the app is deployed on static hosting (like Vercel)
// where the Node/Express server does not run, the AI features work flawlessly!

const SYSTEM_INSTRUCTION_ANALYZE = `너는 자동차 및 글로벌 제조업 공급망 관리(SCM) 및 생산 관리 분야에서 15년 이상의 경력을 가진 "생산 스케줄링 및 자재 수급 최적화 전문가"다.
...
const SYSTEM_INSTRUCTION_CHAT = (part: string, situation: string, report: string) => `너는 SCM 및 생산 관리 분야에서 15년 이상의 경력을 가진 "생산 스케줄링 및 자재 수급 최적화 전문가"다.
...
async function tryModels(apiKey: string, contents: any, systemInstruction?: string): Promise<string> {
  const models = [
    "gemini-2.5-flash",
    "gemini-3.5-flash",
    "gemini-2.0-flash",
  ...
  for (const model of models) {
    try {
      ...
      if (res.ok) {
        ...
        console.warn(`Browser fallback try with model ${model} failed:`, errData);
        lastError = new Error(errData.error?.message || `HTTP ${res.status}`);
      }
    } catch (err: any) {
      console.warn(`Browser fallback try with model ${model} failed:`, err);
      lastError = err;
    }
  }

  throw lastError || new Error("모든 Gemini 모델의 호출에 실패했습니다.");
}

export async function fallbackGeminiAnalyze(part: string, situation: string, apiKey: string): Promise<string> {
...
위 데이터를 바탕으로 최고의 시나리오 대책 보고서를 작성해 주십시오.`;
  ...
  return await tryModels(apiKey, contents, SYSTEM_INSTRUCTION_ANALYZE);
}

export async function fallbackGeminiChat(
  ...
  const systemInstruction = SYSTEM_INSTRUCTION_CHAT(part, situation, report);
  ...
  // Append current message if not already included in history
  ...
  return await tryModels(apiKey, contents, systemInstruction);
}
